(function () {
    ...
    function SearchController($rootScope, $location, LocationService, $sce, $http) {
      ...
      /*  function testjsonp(){
            ...
            $http.jsonp("https://en.wikivoyage.org/w/api.php?action=opensearch&format=json&callback=JSON_CALLBACK&search=bos").then(
                function(response){
                    console.log(response);
                    var data = response.data[1];
                    var locationsarray = [];
                    for(var location in data){
                        locationsarray.push(data[location]);

                    }
                    vm.locations = new Array({"location" : locationsarray});

                    vm.locations = vm.locations[0];
        ...
        for(var j in country_list) {
            vm.loc.push(country_list[j]);
        }
